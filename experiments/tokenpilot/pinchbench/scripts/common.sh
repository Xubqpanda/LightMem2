...
require_method_runtime_env() {
  if [[ -z "${TOKENPILOT_BASE_URL:-}" ]]; then
    printf 'Missing TOKENPILOT_BASE_URL. Define it in experiments/tokenpilot/pinchbench/.env or the shell environment.\n' >&2
    return 1
  fi
  if [[ -z "${TOKENPILOT_API_KEY:-}" ]]; then
    printf 'Missing TOKENPILOT_API_KEY. Define it in experiments/tokenpilot/pinchbench/.env or the shell environment.\n' >&2
    return 1
  fi
}
...
ensure_plugin_runtime_config() {
  ...
  local reduction_pass_format_slimming="${TOKENPILOT_REDUCTION_PASS_FORMAT_SLIMMING:-true}"
  local reduction_pass_format_cleaning="${TOKENPILOT_REDUCTION_PASS_FORMAT_CLEANING:-true}"
  local reduction_pass_path_truncation="${TOKENPILOT_REDUCTION_PASS_PATH_TRUNCATION:-true}"
  local reduction_pass_image_downsample="${TOKENPILOT_REDUCTION_PASS_IMAGE_DOWNSAMPLE:-true}"
  local reduction_pass_line_number_strip="${TOKENPILOT_REDUCTION_PASS_LINE_NUMBER_STRIP:-true}"
